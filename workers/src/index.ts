...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        if (
            env.CORS_ORIGIN !== "*" &&
            request.headers.get("Origin") !== env.CORS_ORIGIN
        ) {
        ...
        if (undefinedParams.length > 0) {
            return new Response(
                `${undefinedParams.join(", ")}が指定されていません`,
                {
                    status: 400,
                    headers: defaultResponseHeaders,
                },
...
const fetchDownloadUrl = async (
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            return {
                ok: false,
                message: `ダウンロードURLを取得中にGitHub APIからエラーが返されました: ${resp.statusText}`,
                status: resp.status,
            };
        }
        ...
        return {
            ok: false,
            message:
                "ダウンロードURLを取得中にGitHub APIへのリクエストでエラーが発生しました",
        };
    }
};
